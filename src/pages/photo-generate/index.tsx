  ...
  const handleGenerate = async () => {
    try {
      if (mode === "humanBg") {
        ...
        await Promise.all([
          putFileToS3(p.presignedUrl, personFile),
          putFileToS3(s.presignedUrl, spotFile),
        ]);
  ...
  if (step === "loading") {
    return (
      <div style={{ ...s.page, alignItems: "center", justifyContent: "center", gap: "24px" }}>
        ...
        <div style={s.spinner} />
        <p style={{ fontSize: "15px", color: C.muted, letterSpacing: "-0.3px", margin: 0 }}>
          사진을 생성 중입니다
        </p>
      </div>
    );
  ...
  if (step === "result") {
    return (
      <div style={s.page}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", padding: "24px 20px 0", gap: "20px" }}>
          <p style={{ fontSize: "18px", fontWeight: 700, color: C.red, letterSpacing: "-0.3px", margin: 0 }}>
            사진이 완성되었습니다!
          </p>
          <div style={{ width: "100%", aspectRatio: "4/3", borderRadius: "8px", border: `3px solid ${C.darkRed}`, overflow: "hidden", backgroundColor: C.surface }}>
            <img src={generatedImage} alt="생성된 이미지" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
          </div>
        </div>

        <div style={{ padding: "20px 20px 40px", display: "flex", flexDirection: "column", gap: "10px" }}>
          <button style={{ width: "100%", height: "48px", border: "1px solid rgba(194,160,158,0.2)", borderRadius: "8px", backgroundColor: "rgba(255,179,173,0.08)", color: C.pink, fontSize: "15px", fontWeight: 600, cursor: "pointer", letterSpacing: "-0.2px" }} onClick={handleGenerate}>↻ 재생성 하기</button>
  ...
  return (
    <div style={s.page}>
      ...
      {errorMsg && (
        <div style={s.overlay}>
          <div style={s.modal}>
            <p style={{ fontSize: "16px", fontWeight: 700, color: "#fff", textAlign: "center", margin: "0 0 8px" }}>잘못 입력하셨어요</p>
            <p style={{ fontSize: "13px", color: C.muted, textAlign: "center", margin: "0 0 20px" }}>{errorMsg}</p>
            <button style={s.mainBtn} onClick={() => setErrorMsg("")}>확인</button>
          </div>
        </div>
      )}
      ...
      <div style={s.content}>
        ...
        {/* 삭제 버튼 */}
        <button style={s.deleteBtn} onClick={resetFiles}>사진 파일 삭제</button>
...
function UploadSlot({
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "6px" }}>
      ...
      <label style={{ display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: C.surface, height: "150px", borderRadius: "4px", cursor: "pointer", overflow: "hidden", border: `1px solid ${C.border}` }}>
        {preview ? (
          <img src={preview} alt={label} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
        ) : (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "8px" }}>
            <div style={{ width: "36px", height: "36px", borderRadius: "50%", backgroundColor: C.border, display: "flex", alignItems: "center", justifyContent: "center" }}>
...
function ChevronLeft() {
  return (
    <svg width="22" height="22" fill="none" stroke={C.pink} strokeWidth="2.5"
